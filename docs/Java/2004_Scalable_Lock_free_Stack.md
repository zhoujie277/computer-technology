...
文献描述了两种基于漏斗树和消除树相结合的高性能并发堆栈算法。不幸的是，漏斗是可线性化但阻塞的，而消除树是非阻塞但不可线性化的。两者都没有在实践中使用，因为它们仅在极高负载下表现良好。文献还描述了一种简单的无锁线性化堆栈算法，该算法在低负载下工作，但不会随着负载的增加而扩展。因此，设计一个非阻塞、可线性化且在整个并发范围内具有良好扩展性的堆栈算法的问题仍然是一个悬而未决的问题。
...
本文提出了一种并行堆栈算法。它基于以下简单的观察结果：用作简单无锁堆栈退避方案的单个消除阵列是无锁、可线性化和可伸缩的。正如我们的实证结果所示，在低负载下，最终消除回退堆栈的性能与简单堆栈一样好，并且随着并发性的增加，其性能越来越优于所有其他方法（基于锁和非阻塞）。我们相信它的简单性和可扩展性使它成为实现并发堆栈的现有构造的可行的实用替代方案。
...
共享堆栈广泛应用于并行应用程序和操作系统中。如中所示，基于后进先出的调度不仅减少了过多的任务创建，而且还防止线程尝试退出队列并执行依赖于其他任务结果的任务。并发共享堆栈是一种数据结构，支持具有线性化后进先出语义的常用推送和弹出操作。线性化保证了操作具有原子性，并且可以以模块化的方式与其他操作组合。
...
当在共享内存机器上运行并行应用程序的线程同时访问共享堆栈对象时，必须使用同步协议来确保正确性。众所周知，多线程并发访问单个对象会导致性能下降。因此，除了正确性之外，同步方法还应该在可伸缩性和健壮性方面提供效率，以应对调度约束。然而，在更常见的低争用情况下，高负载下的可伸缩性不应以良好性能为代价。
...
不幸的是，用于并行化共享堆栈的两种已知方法不符合这些标准。Shavit和Zemach的组合漏斗是可线性化的后进先出堆栈，通过组合提供可伸缩性，但由于组合开销，在低负载下性能较差。它们也是阻塞的，因此在面对调度约束时并不健壮。Shavit 和 Touitou 的消除树是非阻塞的，因此是健壮的，但是它们提供的堆栈是不可线性化的，并且它也有很大的开销，这导致它在低负载下性能不佳。另一方面，Michael 和 Scott 的结果表明，最著名的低负载方法，Treiber 的简单线性化无锁堆栈，由于争用和固有的顺序瓶颈，扩展性较差。
...
本文提出了一种新的并行堆栈算法——消除回退堆栈，它克服了上述方法的综合缺点。该算法是可线性化的，因此易于与其他算法模块化组合，它是无锁的，因此具有鲁棒性，它是并行的，因此具有可扩展性，并且它自适应地利用其并行结构，这使得它能够在低负载下表现良好。消除回退堆栈基于以下简单观察：单个消除阵列（用作无锁堆栈的回退方案）是无锁且可线性化的。在退避过程中引入消除具有增加并行性和减少争用的双重目的，正如我们的实证结果所示，这使得消除退避堆栈在高负载和低负载下都优于文献中的所有算法。我们相信它的简单性和可扩展性使它成为实现并发堆栈的现有构造的可行的实用替代方案。
...
通常，并发数据结构的算法分为两类：阻塞和非阻塞。文献中有几种基于锁的并发堆栈实现。通常，基于锁的堆栈算法预计会提供有限的鲁棒性，因为它们容易受到长延迟和优先级反转的影响。
...
Treiber 提出了第一个基于并发列表的堆栈的非阻塞实现。他将堆栈表示为带有顶部指针的单链表，并使用比较和交换（CAS）以原子方式修改顶部的值。Treiber 没有报告其非阻塞堆栈的性能结果。Michael 和 Scott 将 Treiber 的堆栈与基于 Herlihy 一般方法的优化非阻塞算法以及基于锁的堆栈进行了比较。他们表明，Treiber 算法的总体性能最好，性能差距随着系统中多道程序数量的增加而增大。然而，从他们的性能数据可以清楚地看出，由于固有的顺序瓶颈，Treiber 堆栈提供的可伸缩性很小。
...
Shavit 和 Touitou 引入了消除树（elimination trees），这是一种可伸缩的树状数据结构，其行为“几乎”像堆栈。它们的消除技术（我们将很快详细介绍，因为它是我们新算法的关键）允许高度分布式耦合和操作的执行，具有相反的语义，如堆栈上的push和pops。消除树是无锁的，但不能线性化。以类似的方式，Shavit和Zemach引入了组合漏斗，并使用它们提供可伸缩的堆栈实现。组合漏斗采用组合和消除来提供可伸缩性。它们通过线性化改进了消除树，但不幸的是它们被阻塞了。如前所述，和都是针对高端可扩展性的，这导致了开销，严重影响了它们在低负载下的性能。
...
因此，设计一个在高负载和低负载下都能很好运行的实用无锁线性化并发堆栈的问题仍然没有解决。
...
考虑以下 Shavit 和 Touitou 的简单观察：如果在堆栈上执行 push，紧跟着 pop，则数据结构的状态不会改变（类似于 pop 和 push）。这意味着，如果一对推送和弹出可以在不同的位置相遇和配对，那么线程可以交换值，而不必接触集中式结构，因为它们无论如何都“消除”了彼此对它的影响。消除可以通过使用冲突数组来实现，在该数组中，线程选择随机位置以尝试和冲突。在某个位置“碰撞”的线程对通过无锁同步协议运行，所有这些不相交的碰撞都可以并行执行。如果一个线程在所选位置没有遇到另一个线程，或者它遇到了一个具有无法消除的操作（例如两个推送操作）的线程，则必须使用替代方案。在消除树中，想法是建立消除阵列树，并使用Shavit和Zemach的衍射树范式处理非消除操作。然而，正如我们所注意到的，这种机制的开销很高，并且它们不能线性化。
...
本文中的新思想（见图 1）非常简单：在共享无锁堆栈上使用单个消除数组作为退避方案。如果线程在堆栈上失败，它们将尝试在阵列上消除，如果消除失败，它们将尝试再次访问堆栈，以此类推。令人惊讶的结果是，这种结构是可线性化的：共享堆栈上的任何操作都可以在接入点处线性化，任何消除的操作对在满足时都可以线性化。因为它是一种退避方案，所以它在低负载下提供与简单堆栈相同的性能。但是，与简单堆栈不同，它可以随着负载的增加而扩展，因为（1）成功消除的数量增加，允许许多操作并行完成，和（2）共享堆栈头上的争用减少到最佳指数退避方案可达到的水平之外，因为在阵列中消除了大量退避操作，并且从不重新尝试访问共享结构。
...
我们将新的消除回退堆栈算法与使用 Mellor Crummey 和 Scott 的 MCS 锁的基于锁的实现以及几种非阻塞实现进行了比较：带和不带回退的线性化 Treiber 算法以及 Shavit 和 Touitou 的消除树。我们的比较基于一组在14 节点共享内存机器上执行的合成微基准。我们的结果（见第 4 节）表明，消除回退堆栈优于所有三种方法，特别是两种无锁方法，在峰值负载下表现出几乎三倍的吞吐量。与其他方法不同，它在整个并发范围内保持恒定的延迟，并且在不同推送和弹出比率的实验中也表现良好。
...
我们现在介绍我们的消除回退堆栈算法。图 2 指定了一些类型定义和全局变量。
...
我们现在提供算法的代码。如图 3 和图 4 所示。从代码中可以看出，首先每个线程尝试在中央堆栈对象上执行其操作（第 P1 行）。如果此尝试失败，线程将以下面描述的方式穿过冲突层。
