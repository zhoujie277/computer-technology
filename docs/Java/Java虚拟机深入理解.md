...
##### 试简单描述分代收集的理论
分代收集理论是指通常将堆内存空间划分为被称作新生代和老年代的两个区域，通常情况下，一个对象首先会在新生代区域内分配，然后随着每一次垃圾收集的动作发生，存在于新生代区域的对象会被逐步晋升到老年代中去。另外，通常情况下，每一次垃圾收集，存在于新生代的大部分对象基本都会被回收。只有少部分或没有对象会逐渐晋升到老年代中。即弱分代假说，这个假说很重要，且这个假说成立的程度决定了分代收集器的性能。
