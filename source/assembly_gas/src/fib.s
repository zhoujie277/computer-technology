...
print:
    # We need call printf, but we are using eax, ebx and ecx. printf
    ...
    mov $format, %rdi   # set 1st paramerter(format)
    mov %rax, %rsi      # set 2st parameter(current_number)
    ...
    add %rdx, %rbx      # get the next number
    dec %ecx            # count down
