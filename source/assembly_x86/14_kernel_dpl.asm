...
;字符串显示例程:显示以0为终止字符的字符串, 输入DS:EBX=串地址.
put_string:
        push ecx
    .getc:
        mov cl, [ebx]
...
put_char:
    ...
    .set_cursor:
        ...
        popad
        ret
;从影片读取一个扇区EAX=逻辑扇区号,DS:EBX=目标缓冲区地址
;返回：EBX=EBX+512
read_hard_disk_0:
        push eax
        push ecx
...
;构造存储器和系统的段描述符，
;输入 EAX=线性基地址，EBX=段界限,ECX=属性（各属性都在原始位置，无关位置清零）
;返回 EDX:EAX=描述符
make_seg_descriptor:
        mov edx, eax
        shl eax, 16
        or ax, bx               ;描述符前32位构造完毕
        and edx, 0xffff0000
        rol edx, 8              ;循环左移8位之后，edx的高8位是eax的高16位中的低8位，需要反转才符合传参要求
        ;字节序反转
        bswap edx               ;装配基地址的31～24和23～16
        xor bx, bx
...
;安装门描述符（调用门等）
;输入 EAX=门代码在段内偏移地址 BX=门代码所在段的选择子 CX=段类型及属性等（各属性均在原始位置)
;返回 EDX:EAX=完整的描述符
...
fill_descriptor_in_ldt:
        ...
        mov cx, ax
        shl cx, 3                   ;左移3位，并且使TI=1，指向LDT，最后使RPL=00
...
load_relocate_program:
        ...
        mov ebp, eax
        and ebx, 0xfffffe00             ;使之512字节对齐（能被512整除的数低9位都位0
        add ebx, 512     
        test eax, 0x000001ff            ;程序的大小是512的倍数嘛？
    ...
    .b1:
        ...
        mov eax, 4096
        mul ecx
        mov ecx, eax                    ;准备为栈分配内存
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                    ;得到栈的高端物理地址
        mov ecx, 0x00c0f600             ;字节粒度的栈段描述符，特权级3
        call sys_routine_seg_sel:make_seg_descriptor
        mov ebx, esi                    ;TCB的基地址
        call fill_descriptor_in_ldt
        or cx, 0000_0000_0000_0011B     ;设置选择子的特权级为3
        mov [edi + 0x08], cx            ;登记栈段选择子到头部

        ;重定位salt
        mov eax, mem_0_4_gb_seg_sel     ;头部段描述符一安装，但还没有生效，故只能通过4GB段访问用户程序头部
        mov es, eax
        mov eax, core_data_seg_sel
        mov ds, eax
        cld
    ...
    .b3:
        ...
        repe cmpsd                      ;相等则重复，每次比较4字节
        jnz .b4
        mov eax, [esi]                  ;若匹配，则esi恰好指向其后的地址
        mov [es: edi - 256], eax
        mov ax [esi + 4]
        or ax, 0000_0000_0000_0011B     ;以用户程序自己的特权级使用门，故RPL=3，
    ...
    .b4:
        ...
        ;创建0特权级堆栈
        mov ecx, 4096
        mov eax, ecx                    ;为生产栈高端地址做准备
        mov [es: esi + 0x1a], ecx
        shr dword [es: esi + 0x1a], 12  ;登记0特权级栈尺寸到TCB
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                    ;栈必须使用高端地址为基地址
        mov [es: esi + 0x1e], eax       ;登记0特权级栈基地址到TCB
        mov ebx, 0xffffe                ;段长度(界限)
        mov ecx, 0x00c09600             ;4KB粒度，读写，特权级0
        call sys_routine_seg_sel:make_seg_descriptor
        mov ebx, esi
        call fill_descriptor_in_ldt
        mov [es:esi + 0x22], cx         ;登记0特权级栈选择子到TCB
        mov dword [es:esi + 0x24], 0    ;登记0特权级栈出事ESP到TCB
        ...
        ;创建1特权级堆栈
         ...
         mov dword [es:esi+0x40],0          ;登记2特权级堆栈初始ESP

         ;在GDT中登记LDT描述符
         mov eax, [es: esi + 0x0c]          ;LDT的起始线性地址
         movzx ebx, word [es :esi + 0x0a]   ;LDT段界限
         mov ecx, 0c00408200                ;LDT描述符，特权级0
         call sys_routine_seg_sel:make_seg_descriptor
...
append_to_tcb_link:
        ...
        mov ds, eax
        mov eax, mem_0_4_gb_seg_sel         ;令ES指向0..4GB的段
        mov es, eax
        ;当前TCB指针域清零，以指示这是最后一个TCB。
        mov dword [es: ecx + 0x00], 0        
        mov eax, [tcb_chain]                ;TCB表头指针
        or eax, eax                         ;链表为空？改变标志寄存器，但不改变自身的值
        jz .notcb
    .searc:
        mov edx, eax
        mov eax, [es: edx + 0x00]
...
start:
        ...
        ;以下开始安装位整个系统服务的调用门，特权级之间控制转移必须使用门
    ...
    .b3:
        ...
        ;创建任务控制块，这不是处理器的要求，而是我们为了方便而设立的
        mov ecx, 0x46
        call sys_routine_seg_sel:allocate_memory
