...
mem_0_4_gb_seg_sel  equ 0x08    ;整个0-4GB内存段段的选择子

;核心程序的头部
core_length         dd  core_end    ;核心程序总长度#00
sys_routine_seg_sel dd  section.sys_routine.start   ;系统公用例程段位置#04
...
;终止当前任务
;注：执行此例程时，当前任务仍在运行中。此例程其实也是当前任务的一部分
...
start:
    ...
    .b3:
        ...
        mov cx, 1_11_0_1100_000_00000B  ;特权级3的调用门，0个参数
        call sys_routine_seg_sel:make_gate_descriptor
        call sys_routine_seg_sel:set_up_gdt_descriptor
        mov [edi + 260], cx             ;将返回的门描述符选择子回填
        add edi, salt_item_len          ;下一个C-SALT条目
        pop ecx
        loop .b3
        ...
        call sys_routine_seg_sel:allocate_memory
        mov [prgman_tss + 0x00], ecx    ;保存该程序管理器的TSS基地址
        ...
        mov word [es:ecx + 0], 0        ;反向链=0
        mov dword [es:ecx + 28], 0      ;登记CR3（PDBR）
        mov word [es:ecx + 100], 0      ;T=0,不需要0、1、2特权级栈，0特权级不向低特权级转移控制
        ...
        call far [es: ecx + 0x14]       ;执行任务切换。任务切换时要恢复TSS内容，所以在创建任务时TSS要填写完整
