...
;以下常量定义部分，内核的大部分内容应当固定
core_code_seg_sel equ 0x38      ;内核代码段选择子
core_data_seg_sel equ 0x30      ;内核数据段选择子
sys_routine_seg_sel equ 0x28    ;系统公共历程代码段的选择子
...
;从硬盘读取一个逻辑扇区，EAX=逻辑扇区号，DS:EBX=目标缓冲区地址
;返回：EBX = EBX + 512
read_hard_disk_0:
        push eax
        push ecx
...
put_hex_dword:
        ...
        mov ecx, 8                      ;32位二进制数转成8位16进制
    .xlt:
        rol edx, 4
        mov eax, edx
        and eax, 0x0000000f
        xlat                            ;查表指令,ds:[bx]作为表格，al作为索引，并接收数据
...
allocate_memory:
        ...
        and ebx, 0xfffffffc
        add ebx, 4                  ;强制对齐
        test eax, 0x00000003        ;下次分配的起始弟子最好是4字节对齐
...
;构造存储器和系统的段描述符
;输入 EAX=线性基地址 EBX=段界限 ECX=属性，各属性在原始位置）
;输出 EDX:EAX=描述符
make_seg_descriptor:
        mov edx, eax
        shl eax, 16
...
load_relocate_program:
        ...
        mov ebx, eax
        and ebx, 0xfffffe00         ;使之512字节对齐（能被512整除的数，低9位都位0）
        add ebx, 512
    ...
    .b1:
        ...
        call sys_routine_seg_sel:allocate_memory
        add eax, ecx                ;得到栈段的高端物理地址
        mov ecx, 0x00c09600         ;4KB粒度的栈段描述符
        call sys_routine_seg_sel:make_seg_descriptor
        call sys_routine_seg_sel:set_up_gdt_descriptor
        mov [edi + 0x08], cx
...
return_point:                           ;用户程序返回点
    mov eax, core_data_seg_sel          ;切换数据段和栈段
    mov ds, eax
